        ...
        <script type="text/javascript">
            $(document).ready(function(){
                ...
                $(".edit").click(function () {
                    ...
                    var id = $(this).parent().find('#id').val();
                    var className = $(this).parent().find('#className').val();
                    var maxPart = $(this).parent().find('#maxPart').val();
                    var startDateTime = $(this).parent().find('#startDateTime').val();
                    var stopDateTime = $(this).parent().find('#stopDateTime').val();
                    $('#editClassModal #idEdit').val(id);
                    $('#editClassModal #classNameEdit').val(className);
                    $('#editClassModal #maxPartEdit').val(maxPart);
                    $('#editClassModal #startDateTimeEdit').val(startDateTime);
                    $('#editClassModal #stopDateTimeEdit').val(stopDateTime);
                });
                $(".book").click(function () {
                    console.log("book modal");
                    var id = $(this).parent().find('#id').val();
                    $('#bookClassModal #idBook').val(id);
                });
        ...
        <div class="table-responsive">
            <div class="table-wrapper">
                <div class="table-title">
                    <div class="row">
                        <div class="col-sm-6">
                            <h2>Manage <b>Fitness Classes</b></h2>
                        </div>
                        <div class="col-sm-6">
                            <%--<a href="${pageContext.request.contextPath}/main" class="btn btn-success" ><i class="material-icons">home</i> <span>Home</span></a>--%>
                            <%--<a href="#bookClassModal" class="btn btn-success" data-toggle="modal"><i class="material-icons">date_range</i> <span>Book Class</span></a>--%>
                            <%--<a href="#deleteClassModal" class="btn btn-danger" data-toggle="modal"><i class="material-icons">remove_circle</i> <span>Delete Class</span></a>--%>
                            <a href="#addClassModal" class="btn btn-primary" data-toggle="modal" ${requestScope.powerUser}><i class="material-icons">add_circle</i> <span>Add Class</span></a>
                        </div>
                    </div>
                </div>
                <table class="table table-striped table-hover">
                    ...
                    <tbody>
                    <c:forEach var="fitnessClass" items="${requestScope.fitnessClasses}">
                        <tr>
                            <td>
                                <c:out value="${fitnessClass.className}"/>
                            </td>
                            <td>
                                <c:out value="${fitnessClass.setParticipants}"/>
                            </td>
                            <td>
                                <c:out value="${fitnessClass.registeredParticipants}"/>
                            </td>
                            <td>
                                <c:out value="${fitnessClass.startDateTime}"/>
                            </td>
                            <td>
                                <c:out value="${fitnessClass.endDateTime}"/>
                            </td>
                            <td>
                                <c:out value="${fitnessClass.isActive}"/>
                            </td>
                            <td>
                                <a href="#editClassModal" class="edit" data-toggle="modal" ${requestScope.powerUser}><i class="material-icons" data-toggle="tooltip" title="Edit" data-original-title="Edit"></i></a>
                                <a href="#deleteClassModal" class="delete" data-toggle="modal" ${requestScope.powerUser}><i class="material-icons" data-toggle="tooltip" title="Delete" data-original-title="Delete"></i></a>
                                ...
                                <input type="hidden" name="id" id="id" value="${fitnessClass.id}">
                ...
                <div class="clearfix">
                    <div class="hint-text">Showing <b>5</b> out of <b>${noOfPages}</b> entries</div>
                    <ul class="pagination">
                        <%--Previous link--%>
                        <c:if test="${currentPage != 1}">
                            <li class="page-item disabled"><a href="${pageContext.request.contextPath}/navclasses?page=${currentPage - 1}">Previous</a></li>
                        </c:if>
                        ...
                        <c:forEach begin="1" end="${noOfPages}" var="i">
                            <c:choose>
                                ...
                                <c:otherwise>
                                    <li class="page-item"><a href="${pageContext.request.contextPath}/navclasses?page=${i}" class="page-link">${i}</a></li>
                                </c:otherwise>
                            </c:choose>
                        </c:forEach>
                        <%--Next link--%>
                        <c:if test="${currentPage lt noOfPages}">
                            <li class="page-item disabled"><a href="${pageContext.request.contextPath}/navclasses?page=${currentPage + 1}">Next</a></li>
                        </c:if>
                    </ul>
                </div>
            </div>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <form method="post">
                    ...
                    <div class="modal-header">
                        <h4 class="modal-title">Delete Fitness Class</h4>
                        <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
                    </div>
                    <div class="modal-body">
                        <p>Are you sure you want to delete this Fitness Class?</p>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <form method="post">
                    ...
                    <div class="modal-header">
                        <h4 class="modal-title">Book Fitness Class</h4>
                        <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
                    </div>
                    <div class="modal-body">
                        <p>Please press Book to register to the selected class.</p>
                    </div>
                    <div class="modal-footer">
                        <input type="button" class="btn btn-default" data-dismiss="modal" value="Cancel">
